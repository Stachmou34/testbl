...
# URL pour la recherche d'animes de 2024
...
print('Récupération des animes de 2024...')
while True:
    params = {
    ...
    for entry in data.get('data', []):
        ...
        # Vérifier si l'anime est de type TV et de 2024
        start_date = anime.get('start_date', '')
        if (anime.get('media_type') == 'tv' and 
            start_date.startswith('2024')):
    ...
    # Si on a déjà plus de 50 animes TV de 2024, on peut arrêter
    if len(animes) >= 50:
        break
...
if not animes:
    print('Aucun anime TV de 2024 trouvé. Vérifiez la réponse ci-dessus ou le token.')
else:
    print(f'{len(animes)} animes TV de 2024 exportés dans {csv_file}') 