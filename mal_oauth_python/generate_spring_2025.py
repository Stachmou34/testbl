...
df = pd.read_csv('animes_spring_2025.csv')
...
def create_card(row):
    ...
    score_txt = f"Score: {row['num_list_users']:.2f}" if pd.notna(row['num_list_users']) and row['num_list_users'] > 0 else "Score: N/A"
